...
public class Exercises {
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		List<Double> dividedBy2 = new ArrayList<Double>();
		for (Integer tobeDivided : intArray) {
			dividedBy2.add(tobeDivided / 2.0);
		}
		return dividedBy2;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		for (Integer doesItAppearTwice : integerList) {
			if (intToFind == doesItAppearTwice) {
				count++;
			}
		}
		if (count == 2) {
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (Integer integerValues : integerArray) {
			if (integerValues % 3 == 0 && integerValues % 5 == 0) {
				fizzBuzzStringList.add("FizzBuzz");
			} else if (integerValues % 3 == 0) {
				fizzBuzzStringList.add("Fizz");
			} else if (integerValues % 5 == 0) {
				fizzBuzzStringList.add("Buzz");
			} else {
				fizzBuzzStringList.add(integerValues.toString());
			}
		}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		List<Integer> interleavedLists = new ArrayList<Integer>();
		int size = Math.max(listOne.size(), listTwo.size());

		for (int i = 0; i < size; i++) {
			if (i < listOne.size()) {
				interleavedLists.add(listOne.get(i));
			}
			if (i < listTwo.size()) {
				interleavedLists.add(listTwo.get(i));
			}
		}
